...
def _iter_prebuilt_library_paths():
    ...
    for libname in (
        "libsecp256k1_" + system + "_" + machine + ext,
        "libsecp256k1" + ext,
        "libsecp256k1.0.dylib",
        "libsecp256k1.so.0",
    ):
...
# ecdh
@locked
def ecdh(pubkey, scalar, hashfn=None, data=None, context=None):
    if context is None:
        context = _secp.ctx
...
def schnorrsig_sign(
    msg, keypair, nonce_function=None, extra_data=None, context=None
):
    if len(msg) != 32:
        raise ValueError("Message should be 32 bytes long")
    if len(keypair) == 32:
...
@locked
def pedersen_commitment_parse(inp, context=None):
    if context is None:
        context = _secp.ctx
    if len(inp) != 33:
...
@locked
def pedersen_commitment_serialize(commit, context=None):
    if context is None:
        context = _secp.ctx
    if len(commit) != 64:
...
@locked
def pedersen_commit(vbf, value, gen, context=None):
    if context is None:
        context = _secp.ctx
    if len(gen) != 64:
        raise ValueError("Generator should be 64 bytes long")
    if len(vbf) != 32:
        raise ValueError("Blinding factor should be 32 bytes long, not " + str(len(vbf)))
    commit = bytes(64)
...
def pedersen_blind_generator_blind_sum(
    values, gens, vbfs, num_inputs, context=None
):
    if context is None:
        context = _secp.ctx
...
@locked
def pedersen_verify_tally(ins, outs, context=None):
    if context is None:
        context = _secp.ctx
...
# generator
@locked
def generator_parse(inp, context=None):
    if context is None:
        context = _secp.ctx
    if len(inp) != 33:
...
@locked
def generator_generate(asset, context=None):
    if context is None:
        context = _secp.ctx
    if len(asset) != 32:
...
@locked
def generator_generate_blinded(asset, abf, context=None):
    if context is None:
        context = _secp.ctx
    if len(asset) != 32:
        raise ValueError("Asset should be 32 bytes long")
...
@locked
def generator_serialize(generator, context=None):
    if context is None:
        context = _secp.ctx
    if len(generator) != 64:
        raise ValueError("Generator should be 64 bytes long")
...
def rangeproof_verify(
    proof, value_commitment, script_pubkey, generator, context=None
):
    if len(generator) != 64:
        raise ValueError("Generator should be 64 bytes long")
    if len(value_commitment) != 64:
...
def surjectionproof_initialize(
    in_tags, out_tag, seed, tags_to_use=None, iterations=100, context=None
):
...
def surjectionproof_generate(
    proof, in_idx, in_tags, out_tag, in_abf, out_abf, context=None
):
...
@locked
def surjectionproof_verify(proof, in_tags, out_tag, context=None):
    if context is None:
        context = _secp.ctx
...
@locked
def surjectionproof_serialize(proof, context=None):
    if context is None:
        context = _secp.ctx
...
@locked
def surjectionproof_parse(proof, context=None):
    if context is None:
        context = _secp.ctx
