...
def _iter_prebuilt_library_paths():
    ...
    for libname in (
        "libsecp256k1_{system}_{machine}{ext}".format(system=system, machine=machine, ext=ext),
        "libsecp256k1{ext}".format(ext=ext),
        "libsecp256k1.0.dylib",
        "libsecp256k1.so.0",
    ):
...
def ec_pubkey_combine(*args):
    ...
    r = _secp.secp256k1_ec_pubkey_combine(_secp.ctx, pub, pubkeys, len(args))
    if r == 0:
...
def musig_pubkey_combine(*args):
    ...
    res = _secp.secp256k1_musig_pubkey_combine(
        _secp.ctx, None, pub, None, pubkeys, len(args)
    )
    if res == 0:
        raise ValueError("Failed to combine pubkeys")
    return pub
